...
def train(net, loaders, args):
    ...
    for epoch in range(args['epochs']):
        ...
        for i, (geometry, damage, imp_shrinkage, _, _) in enumerate(loaders['train']):
            ...
            l_seq = 0
            seq = []
            noise = 0.2*torch.rand_like(damage) - 0.1
            for n in range(10):
                if n == 0:
                    x = torch.cat([geometry, 
                                   imp_shrinkage[:,[1],:,:] / -0.001, 
                                   torch.clamp(damage[:,[0],:,:] + noise[:,[0],:,:], 0., 1.)], axis=1)
                else:
                    x = torch.cat([geometry, imp_shrinkage[:,[n+1],:,:] / -0.001, torch.clamp(y.detach() + noise[:,[n],:,:], 0., 1.)], axis=1)
                # apply the network
                y = net(x)
