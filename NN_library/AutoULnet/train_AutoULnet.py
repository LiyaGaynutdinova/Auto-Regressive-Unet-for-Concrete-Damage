    ...
    for epoch in range(args['epochs']):
        ...
        for i, (geometry, damage, imp_shrinkage, obs_shrinkage, stiffness) in enumerate(loaders['train']):
            ...
            for n in range(10):
                ...
                l_stiff = loss((y[1]*stiffness[:,0].detach()), stiffness[:,n+1]).sum()
                l_shr = (loss(y[2], obs_shrinkage[:,n+1]) / torch.abs(imp_shrinkage[:,n+1,0,0].detach())).sum()
        ...
        for j, (geometry, damage, imp_shrinkage, obs_shrinkage, stiffness) in enumerate(loaders['val']):
            ...
            for n in range(10):
                ...
                l_stiff_val = loss((y[1]*stiffness[:,0].detach()), 
                                             stiffness[:,n+1]).sum().detach().cpu()
                l_shr_val = (loss(y[2], obs_shrinkage[:,n+1]) / imp_shrinkage[:,n+1,0,0].detach().abs()).sum().detach().cpu()
                L_val += l_dam_val + l_shr_val + l_stiff_val

                if j == 0:
                    seq_val.append(y[0][0,0].detach().cpu())
            if j == 0:
