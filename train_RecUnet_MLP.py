...
def train(net, loaders, args):
    ...
    for epoch in range(args['epochs']):
        ...
        for i, (geometry, damage, imp_shrinkage, obs_shrinkage, stiffness) in enumerate(loaders['train']):
            ...
            imp_shrinkage = imp_shrinkage[:,:,0,0].to(args['dev'])
            ...
            for n in range(10):
                if n == 0:
                    x = torch.cat([geometry, damage[:,[n],:,:]], axis=1)
                else:
                    x = torch.cat([geometry, y_dam.detach()], axis=1)
                # apply the network
                y_dam, y_shr = net(x, imp_shrinkage[:,[n]])
                # calculate mini-batch losses
                l_dam = loss_damage(y_dam, damage[:,[n+1],:,:]).sum()
                l_shr = loss_shrinkage(y_shr[:,0]*imp_shrinkage[:,n].detach(), obs_shrinkage[:,n+1]).sum()
                l_stiff = loss_shrinkage(y_shr[:,1]*stiffness[:,0].detach(), stiffness[:,n+1]).sum()
                l = l_dam + l_shr + l_stiff
                #l = l_dam + 0.25*l_shr/l_shr.detach()*l_dam.detach() + 0.25*l_stiff/l_stiff.detach()*l_dam.detach()
                ...
                if i % 100 == 0:
                    seq.append(y_dam[0,0].detach().cpu())
        ...
        for j, (geometry, damage, imp_shrinkage, obs_shrinkage, stiffness) in enumerate(loaders['val']):
            ...
            for n in range(10):
                if n == 0:
                    ...
                    x = torch.cat([geometry, y_dam.detach()], axis=1)
                y_dam, y_shr = net(x, imp_shrinkage[:,[n]])
                l_dam_val = loss_damage(y_dam, damage[:,[n+1],:,:]).sum().detach().cpu().item()
                l_shr_val = loss_shrinkage(y_shr[:,0]*imp_shrinkage[:,n].detach(), obs_shrinkage[:,n+1]).sum().detach().cpu().item()
                l_stiff_val = loss_shrinkage(y_shr[:,1]*stiffness[:,[0]].detach(), stiffness[:,n+1]).sum().detach().cpu().item()
                #L_val += l_dam_val + 0.25*l_shr_val/l_shr_val*l_dam_val + 0.25*l_stiff_val/l_shr_val*l_dam_val
